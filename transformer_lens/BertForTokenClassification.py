...
class BertForTokenClassification:
    """A BERT-style model for token classification tasks.
    """
    ...
    ) -> Optional[Union[Float[torch.Tensor, "batch n_class"], List[str]]]:
        """
        """
        return self.forward(
            input,
    ...
    def forward(
        ...
        token_type_ids: Optional[Int[torch.Tensor, "batch pos"]] = None,
        one_zero_attention_mask: Optional[Int[torch.Tensor, "batch pos"]] = None,
    ) -> Union[Float[torch.Tensor, "batch n_class"], List[str]]:
        ...
    ...
    def forward(
        ...
        # Classification
        logits = self.model.classifier(resid)
        prediction_classes = self.model.cfg.id2label
        if return_type == "predictions":
            return [prediction_classes[str(id.item())] for id in logits.argmax(dim=-1)[0]]
    ...
    ) -> Tuple[Float[torch.Tensor, "batch n_class"], ActivationCache,]:
        ...

    @overload
    def run_with_cache(
        self, *model_args, return_cache_object: Literal[False], **kwargs
    ) -> Tuple[Float[torch.Tensor, "batch n_class"], Dict[str, torch.Tensor],]:
        ...
    ...
    def run_with_cache(
        ...
        **kwargs,
    ) -> Tuple[Float[torch.Tensor, "batch n_class"], Union[ActivationCache, Dict[str, torch.Tensor]],]:
        """
        ...
        class ForwardWrapper:
            def __init__(self, tclassifier):
                self.tclassifier = tclassifier
                self.original_forward = tclassifier.model.forward

            def __enter__(self):
                # Store reference to wrapper function
                def wrapped_forward(*fargs, **fkwargs):
                    return self.tclassifier.forward(*fargs, **fkwargs)

                self.tclassifier.model.forward = wrapped_forward
                return self

            def __exit__(self, exc_type, exc_val, exc_tb):
                # Restore original forward
                self.tclassifier.model.forward = self.original_forward
